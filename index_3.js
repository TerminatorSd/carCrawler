...
var url = 'https://car.autohome.com.cn/pic/series/2288-1-p1.html' 
var saveFile = './res_3.txt'

getAllHtml(urlBase);

function getAllHtml(urlBase) {
  var maxIndex = 2000;
  var startIndex = 3096,
      nowIndex = startIndex;
  ...
  var intId = setInterval(function() {
    if(nowIndex != 2288) {
      ...
      nowIndex++;
      if(nowIndex > startIndex + maxIndex) {
        clearInterval(intId);
      }
    }
  }, 4000)
  ...
  https.get(url, function(res){
    ...
    res.on('end', function() {
      ...
      if (nowImgStr) {
        ...
        fs.writeFile(saveFile, nowImgStr, { 'flag': 'a' }, function(err, data) {
          if(err) {
            console.log('Failure.');
          } else {
            console.log('Saved.')
